...
This blog post details how to setup Twilio functions and use them for verify phone numbers through WhatsApp in your React app.
...
```
The `fetOtp` function calls an `api` route that will in turn call the relevant Twilio function.
```ts
...
> ![get auth token from twilio console](/images/twilio-console-auth-token)